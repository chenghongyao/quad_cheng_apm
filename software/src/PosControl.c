...
//	pos_ctrl.throttle_hover  = 
	//pos_ctrl.dt = 0.01;				//100Hz
	pos_ctrl.limit.pos_up = 0;		//达到了最大高度标志
	pos_ctrl.alt_max = 0;			//无高度限制
	pos_ctrl.last_update_z_ms = 0;
	//pos_ctrl.flags.recalc_leash_z = 0;	//1,关闭前馈,?
...
void pos_control_init_takeoff(void)
{
	...
	pos_ctrl.pid_alt_accel->integrator = apmotor.rc_throttle->servo_out - pos_ctrl.throttle_hover;//加速度初始积分值?
}
...
{
	if (pos_ctrl.flags.recalc_leash_z)		//需要重新计算what
	{
...
void pos_control_accel_to_throttle(float accel_target_z)
{
	...
	z_accel_meas = -(ahrs.accel_ef.z + GRAVITY_MSS) * 100.0f;//当前加速度,改为向上为正!!!????
